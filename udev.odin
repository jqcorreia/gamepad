...
foreign _udev {
	...
	list_entry_get_next :: proc(_: ^udev_list_entry) -> ^udev_list_entry ---
	list_entry_get_by_name :: proc(_: ^udev_list_entry, _: cstring) -> ^udev_list_entry ---
	list_entry_get_name :: proc(_: ^udev_list_entry) -> cstring ---
	list_entry_get_value :: proc(_: ^udev_list_entry) -> cstring ---
	enumerate_new :: proc(_: ^udev) -> ^udev_enumerate ---
	enumerate_unref :: proc(_: ^udev_enumerate) ---
	enumerate_get_udev :: proc(_: ^udev_enumerate) -> ^udev ---
	enumerate_scan_devices :: proc(_: ^udev_enumerate) -> c.int ---
	enumerate_get_list_entry :: proc(_: ^udev_enumerate) -> ^udev_list_entry ---
	enumerate_add_match_subsystem :: proc(_: ^udev_enumerate, _: cstring) -> c.int ---
	device_new_from_syspath :: proc(_: ^udev, _: cstring) -> ^udev_device ---
	device_unref :: proc(_: ^udev_device) ---
	device_get_devnode :: proc(_: ^udev_device) -> cstring ---
	device_get_property_value :: proc(_: ^udev_device, _: cstring) -> cstring ---
	device_get_devnum :: proc(_: ^udev_device) -> dev_t ---
	device_get_properties_list_entry :: proc(_: ^udev_device) -> ^udev_list_entry ---
...
major :: proc(val: dev_t) -> c.int {
	return (val >> 8) & 0xfff // For evdev devices this will always be
}

minor :: proc(val: dev_t) -> c.int {
	return (val & 0xff) | ((val >> 12) & 0xfff00)
}
